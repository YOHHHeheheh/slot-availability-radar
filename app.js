...
function initTrainListing() {
  ...
  document.getElementById('summary-meta').textContent = `${formatDate(search.date)} · ${search.travelClass || 'All Classes'}`;
...
function initTravellersForm() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    selected.search.travellers = travellers;
    selected.totalPrice = selected.pricePerPerson * travellers.length;
    BookingStore.saveSelected(selected);
...
function initPaymentPage() {
  ...
  const selected = BookingStore.getSelected();
  if (!selected || !selected.search.travellers) {
    window.location.href = 'index.html';
    return;
  }
...
function initConfirmationPage() {
  const pnrDisplay = document.getElementById('confirm-pnr');
  if (!pnrDisplay) return;
  ...
  pnrDisplay.textContent = booking.pnr;
}
